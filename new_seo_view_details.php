...
//		$domain_history		=	checkSemarshDomainHistoryData($domain_details['user_id'], $domain_details['domain_url'], $_REQUEST['id']);
	...
	$new_arr[]		=	array(19,31504470,130638193,86905921,855962,25237906,18531300,20181215);
	$new_arr[]		=	array(19,31428334,130546351,87216370,781997,24688558,18258152,20181115);
	$new_arr[]		=	array(20,34087220,132189007,87847948,681771,18793935,14881683,20181015);
	$new_arr[]		=	array(24,36261896,108559244,79568188,596593,18348335,13814442,20180915);
	$new_arr[]		=	array(17,35872063,145322220,98773137,459219,17132356,11858911,20180815);
	$new_arr[]		=	array(22,31971728,126192213,83310184,396048,14714089,10535151,20180715);
	$new_arr[]		=	array(22,32150402,129064157,85648349,656471,17195723,11821757,20180615);
	$new_arr[]		=	array(22,32233111,129012619,87024318,572933,16868825,11620251,20180515);
	$new_arr[]		=	array(22,33578371,129326683,86572478,568862,16842916,11609684,20180415);
	$new_arr[]		=	array(23,34324832,128669039,85257252,565697,16847760,11621421,20180315);


	foreach($new_arr as $arr){
		$timeZone 			= 	new DateTimeZone("Asia/Kolkata");
		$dateTime 			= 	\DateTime::createFromFormat('Ymd', $arr[7])->format('U') * 1000;
//			$date_time			.=	date('Y-m-d', strtotime($dateTime)).',';
		$rank[]				=	array($dateTime, intval($arr[0]));
		$organic_keywordss[]	=	array($dateTime, intval($arr[1]));
		$organic_traffic[]	=	array($dateTime, intval($arr[2]));
		$organic_cost[]		=	array($dateTime, intval($arr[3]));
		$adwords_keywords[]	=	array($dateTime, intval($arr[4]));
		$adwords_traffic[]	=	array($dateTime, intval($arr[5]));
		$adwords_cost[]		=	array($dateTime, intval($arr[6]));
	}
...
<div class="main-container clearfix">
		...
		<div class="page m134 page-charts-c3" ng-controller="c3ChartDemoCtrl">
			...
			<?php if(!empty($profile_info)) { ?>
                <div class="client-share-info">
					...
					?>
                       ...
                       <div class="client-share-logo" style="background-image: url('<?php echo $path.$files[2]; ?>');"></div>
                    </div>
			...
			<?php } ?>
				   ...
				   <div class="pd-fixed-bar-outer" >
                            <div class="pd-fixed-bar"  data-step="1" data-intro="This area shows important statistics like growth in Google traffic, number of keywords, backlinks and goals completed!">
                                <div class="pd-bar-data" >
                                    <a href="#pdS"><h5><div class="custom-circle"></div>Organic keywords</h5>
									<?php $organic_keywords	= round((count($data)-$last_month)/$last_month*100, 2); ?>
                                    <big><?php echo $google_analyics_last; ?> <span id="traffic_growth" class="<?php echo ($google_analyics_second_last < 0 ? 'red' : 'green' ); ?>"><?php echo ($google_analyics_second_last > 0 ? '+' : '' );  echo $google_analyics_second_last."%"; ?></span></big><strong>Keywords</strong></a>
                                </div>
                                <div class="pd-bar-data" >
					...
					<div class="col-md-12">
						<div class="panel panel-default panel-stacked panel-hovered mb30">
							<div class="panel-heading inner-dash-heading">
                                ...
                                <h2 class="text-left ">Domain : <?php echo urlToDomain($domain_details['domain_url']);?></h2>
                            </div>
							...
							<div class="panel-body custom-top-heading-pd">
								...
								<div class="row ">
									<div class="col-md-12 pd-add-pt" data-step="2" data-intro="This is how the number of keywords you are ranking for has been growing. More keywords leads to more traffic">
									...
									<div class="col-md-12" data-step="3" data-intro="This is how your traffic is growing in last 6 months">
				...
				<div class="row" data-step="4" data-intro="These are keywords you are ranking for and some other important metrics">
				...
				<div class="row">
					<div class="blank-link"  id="pdFT"></div>
						...
						<!-- Line Chart -->
						<div class="col-md-12"  data-step="6" data-intro="This area shows how many goals has been completed in Google analytics, don't worry if you haven't set goals as off now, we will do it for you.">
							<div class="panel panel-default panel-stacked panel-hovered mb30">
	...
	<link rel="stylesheet" href="assets/styles/introjs.css"></link>
	<style type="text/css">
		.bs-example{
		margin: 20px;
		}
