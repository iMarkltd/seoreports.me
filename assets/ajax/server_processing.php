...
$total_stmt = $DBcon->prepare("SELECT COUNT(*) AS allcount FROM semrush_users_account WHERE user_id=:user_id AND status=0 ");
...
$fstmt = $DBcon->prepare("SELECT COUNT(*) AS allcount FROM semrush_users_account sua WHERE sua.user_id=:user_id AND sua.status=0 ".$searchQuery);
...
## Fetch records
	...
	$query = "SELECT sua.* FROM semrush_users_account sua WHERE sua.user_id=:user_id AND sua.status=0 ".$searchQuery."  ORDER BY sua.id  asc LIMIT ".$_REQUEST['start']." ,".$_REQUEST['length']." ";
	...
	foreach($results as $result) {
		$totalKeywords      =   countKeywords($result['id']);
		$totalTraffic		=   countTraffic($result['id']);
