...
class SemRush
{
	...
	//protected $apikey='8cf3f5d7d856050d42945ce437aebd91';
	protected $apikey='4db1115329e0af57fffb04585e81b549';
	...
	*/
	...
	public function getDomainHistory($semrushdb)
	{
		if(!empty($semrushdb)) {
			$this->db =	$semrushdb;		
		}
		return $this->callReport('domain_rank_history', $optionalParams=null);
	}
	...
	public function getOrganicKeywordsReport($semrushdb)
	{
		if(!empty($semrushdb)) {
			$this->db =	$semrushdb;		
		}
	...
	public function getBacklinkReport()
	{
		return $this->callReport('backlinks');
	}
	...
	{
		if($reportType == 'backlinks'){
			$domain_url = preg_replace("(^https?://)", "", $this->url );
			$url = 'http://api.semrush.com/analytics/v1/?key=4db1115329e0af57fffb04585e81b549&target='.$domain_url.'&type=backlinks&target_type=root_domain&export_columns=page_score,page_trust_score,response_code,source_size,external_num,internal_num,redirect_url,source_url,source_title,image_url,target_url,target_title,anchor,image_alt,last_seen,first_seen,nofollow,form,frame,image,sitewide,newlink,lostlink&display_limit=10'; 
			//http://api.semrush.com/?type=domain_rank_history&database=us&key=4db1115329e0af57fffb04585e81b549&export=api&export_escape=1&display_limit=12&domain=https://thevandewater.com/			
			//$url = 'http://api.semrush.com/analytics/v1/?key=8cf3f5d7d856050d42945ce437aebd91&target=gamimarket.com&type=backlinks&target_type=root_domain'; 
		}else {
	...
	protected function buildQueryString($reportType, $optionalParams=array())
	{
		...
		{
			if($i == 1)
				$query .= '?' . $paramName . '=' . $paramValue . '&database='. $this->db ;
			else
